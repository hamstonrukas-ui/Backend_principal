...
app.post('/api/tournaments/:tournamentId/join', authenticateToken, async (req, res) => {
  try {
    ...
    const insertResult = await pool.query(
      `INSERT INTO tournament_participants (tournament_id, user_uuid, code_parrainage) VALUES ($1, $2, $3) RETURNING *`,
      [tournamentId, userUuid, monCode]
    );
...
app.get('/api/tournaments/:tournamentId/today', authenticateToken, async (req, res) => {
  try {
    ...
    if (jourActuel < 1) return res.json({ statut: 'pas_commence', jour_debut: 1 });
