...
app.post('/api/tournaments/:tournamentId/join', authenticateToken, async (req, res) => {
  try {
    const { tournamentId } = req.params;
    const { code_parrain, whatsapp_numero } = req.body;
    ...
    const insertResult = await pool.query(
      `INSERT INTO tournament_participants (tournament_id, user_uuid, whatsapp_numero, code_parrainage) VALUES ($1, $2, $3, $4) RETURNING *`,
      [tournamentId, userUuid, whatsapp_numero.trim(), monCode]
    );
...
app.get('/api/tournaments/:tournamentId/leaderboard', authenticateToken, async (req, res) => {
  try {
    const { tournamentId } = req.params;
    const result = await pool.query(
      `SELECT tp.user_uuid, u.name, tp.jours_completes, tp.streak_actuel, tp.points_total
       FROM tournament_participants tp
       JOIN users u ON u.uuid = tp.user_uuid
