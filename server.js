...
async function getTranslations() {
  // Le vrai traducteur (service séparé, server__7_.js) écrit directement
  // dans translation_history à chaque appel — une ligne par requête, sans
  // déduplication. On agrège ici côté lecture par phrase source pour avoir
  // un compteur de popularité, sans avoir à toucher au service de traduction.
  const result = await pool.query(`
    SELECT
      MIN(id) AS id,
      input_text AS sourcetext,
      (array_agg(output_text ORDER BY createdAt DESC))[1] AS translatedtext,
      COUNT(*) AS requestcount,
      COUNT(DISTINCT userUuid) AS uniqueusers,
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (nick.length < 3) {
      return res.status(400).json({ error: 'Le nickname doit contenir au moins 3 caractères.' });
    }
...
app.get('/api/admin/translations', async (req, res) => {
  ...
  try {
    ...
    // Déjà triées par requestcount DESC dans getTranslations(), mais on
    // retrie ici au cas où l'ordre SQL ne serait pas garanti après agrégation.
    translations.sort((a, b) => (b.requestcount || 1) - (a.requestcount || 1));

    const totalRow = await pool.query('SELECT COUNT(DISTINCT userUuid) AS count FROM translation_history');

    const stats = {
      total: translations.length, // phrases distinctes
      totalRequests: translations.reduce((sum, t) => sum + Number(t.requestcount || 1), 0), // toutes demandes confondues
      uniqueUsers: parseInt(totalRow.rows[0].count, 10),
