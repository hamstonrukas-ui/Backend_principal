...
// POST /api/notifications/send-daily — déclenché une fois par jour par un
// service externe (cron-job.org) puisque Render (gratuit) s'endort et ne
// peut pas garantir un minuteur interne fiable. Protégé par une clé secrète.
// Accepte GET et POST : GET permet de tester directement en collant
// l'URL dans un navigateur (utile pour vérifier que ça marche), POST
// reste la méthode "propre" pour un vrai déclenchement automatique.
app.all('/api/notifications/send-daily', async (req, res) => {
  ...
  try {
    ...
    console.error('Erreur /api/notifications/send-daily:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
